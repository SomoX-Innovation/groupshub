...
export async function toggleApproval(id: string, currentlyApproved: boolean) {
  const supabase = createAdminClient()
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await (supabase as any).from('groups').update({ is_approved: !currentlyApproved }).eq('id', id)
  revalidatePath('/admin/groups')
}
